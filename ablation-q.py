...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.aa_input['input_ids'][index], self.aa_input['attention_mask'][index], self.y_data[index]
...
#     df_test_set = df_test_set[-50:]
    ...
    for epoch in range(args.epochs):
        ...
        for i, (train_aa_input_ids, train_aa_attention_mask, train_labels) in enumerate(train_loader):
            ...
            output = model( train_ids_aa, train_mask_aa)
        ...
        for i, (val_aa_input_ids, val_aa_attention_mask, val_labels) in enumerate(val_loader):
            ...
            with torch.no_grad():
                validate_output = model(validate_ids_aa, validate_mask_aa)
    ...
    test_pred = []
    test_true = []
    for i, (test_aa_input_ids, test_aa_attention_mask, test_labels) in enumerate(test_loader):
        ...
        with torch.no_grad():
            test_output = model(test_ids_aa, test_mask_aa)
